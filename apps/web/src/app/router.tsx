...
function LocalizedHome({ locale }: { locale: Locale }): React.JSX.Element {
  ...
  return <HomePage locale={locale} onReportRoute={(path, report, errorCode) => { void navigate(path, { state: { report, ...(errorCode === undefined ? {} : { errorCode }) } }); }} />;
}
...
function RepositoryRoute(): React.JSX.Element {
  ...
  const initialErrorCode = typeof candidateError === "string" && publicErrors.includes(candidateError as PublicErrorCode)
    ...
    : undefined;
  return <ScanPage {...(isReportSnapshot(candidate) ? { initialReport: candidate, ...(initialErrorCode === undefined ? {} : { initialErrorCode }) } : {})} locale={locale} owner={owner} repo={repo} />;
}
