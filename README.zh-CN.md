...
- 展示公开计数、数据可用情况、仓库类型校正、缺失项、反向证据和可检索的 Evidence ID。缺失值留空，不用零代替。
- 用户可自愿登录并调用自己的 GitHub Copilot 权益，对有限的证据包做补充分析；没有 Copilot 也不影响基础报告。
- 在本地导出分享卡片 PNG 或完整证据 JSON；分享链接会重新扫描，不会把 AI 报告永久公开托管。
...
仓库类型判断不明确时也不计分。“证据充分度”是启发式估计，不是准确率或数据覆盖率。五维水位表示实验性风险，越高越值得核对，并不是越高质量越好。Copilot 的“无法验证”和“不适用”不会抬高置信度；没有可计分判断时，基础报告保持不变。
...
<details>
<summary>证据检索与来源核对</summary>

![DAYU 中文证据浏览器](docs/assets/dayu-evidence-zh-desktop.png)
